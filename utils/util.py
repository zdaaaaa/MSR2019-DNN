...
def RMSE(input, target, mmn):
    rmse = torch.sqrt(F.mse_loss(input, target)) * (mmn.max - mmn.min) / 2
...
def RMSE_fix(input, target, mmn):
    rmse = torch.sqrt(F.mse_loss(input, target)) * (mmn.max - mmn.min)
    return rmse